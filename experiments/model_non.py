...
def convlayer(x, filters, activation, name, residual=None, use_instancenorm=True):
    ...
    if use_instancenorm:
        x = GroupNormalization(name=name+"_InstanceNorm", groups=1)(x)
...
def decoder(x, filters, layers, to_concat, name, activation):
    ...
    for i in range(layers):
        ...
        # For the last layer's second convolution, don't use normalization
        if i == layers-1:
            # Last layer of decoder - use convolution without normalization
            x = Conv2D(filters*2**(layers-1-i), 3, padding='same', use_bias=False, 
                      name=f"{name}_dec_layer{layers-i}_conv2")(x)
            x = Activation(activation, name=f"{name}_dec_layer{layers-i}_conv2_activation")(x)
        else:
            # Other layers - use normal convlayer with normalization
